...
restart = 0
steps = 1000000  # used to be 25000, now change to a value that is close to infinity
save_freq = 500
save_max = 20
print_freq = 200
wall_time = 23*60*60
...
def main():
    ...
    while solver.ok:
        if solver.iteration % print_freq == 0:
            max_speed = u['g'].max()
            volume = f.integrate('x', 'z')['g'][0, 0]
            logger.info('{:0>6d}, u max {:f}, dt {:.5f}, time {:.2f}, ice volume {:.3f}'.format(
                solver.iteration, max_speed, dt, (time.time()-start_time)/60, volume))
